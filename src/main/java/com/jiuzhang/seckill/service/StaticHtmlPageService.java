...
@Slf4j
@Service
public class StaticHtmlPageService {
    ...
    /**
     * 创建指定秒杀活动详情页的静态 HTML 页面
     * （按需手动执行 SeckillApplicationTest -> createSeckillItemStaticHtmlPage()）
     */
    public void createSeckillItemStaticHtmlPage(long seckillActivityId) {
        ...
        try {
            ...
            log.error("页面静态化出错：seckillActivityId=" + seckillActivityId);
        } finally {
            if (writer != null) {
                writer.close();
            }
